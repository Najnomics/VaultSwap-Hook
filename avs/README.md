# LVR Auction Hook AVS

A Hourglass-based Autonomous Verifiable Service (AVS) that provides distributed compute infrastructure for Loss-Versus-Rebalancing (LVR) auction operations on Uniswap V4 pools.

## Overview

This AVS serves as a **connector and coordinator** that interfaces with the main LVR Auction Hook project. It implements a task-based architecture using the Hourglass framework to provide:

- **Distributed task execution** for LVR auction operations
...
**Important**: This AVS does **not** contain the auction business logic itself. It provides the distributed compute layer that interfaces with the main [LVR Auction Hook](../src/LVRAuctionHook.sol) contract where all auction logic resides.
...
│   ├── main.go                  # ✅ LVR Auction Performer implementation
...
│   │   │   └── LVRAuctionServiceManager.sol # ✅ L1 connector only
│   │   └── l2-contracts/        # Task lifecycle management (connectors)
│   │       └── LVRAuctionTaskHook.sol # ✅ L2 connector only
│   ├── script/                  # Deployment scripts for connectors
│   │   ├── DeployLVRL1Contracts.s.sol # ✅ Deploy L1 connector
│   │   └── DeployLVRL2Contracts.s.sol # ✅ Deploy L2 connector
│   └── test/                    # Connector contract tests
│       ├── LVRAuctionServiceManager.t.sol # ✅ L1 connector tests
│       └── LVRAuctionTaskHook.t.sol       # ✅ L2 connector tests
...
- Main LVR Auction Hook logic → `../src/LVRAuctionHook.sol`
