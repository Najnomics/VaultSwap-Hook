...
import {LVRAuctionTaskHook} from "@project/l2-contracts/LVRAuctionTaskHook.sol";

contract DeployLVRL2Contracts is Script {
    ...
    struct Context {
        ...
        address lvrAuctionHook;  // Address of the main LVR Auction Hook (deployed separately)
    ...
    function run(string memory environment, string memory _context) public {
        ...
        // Deploy LVR Auction Task Hook (connector to main hook)
        LVRAuctionTaskHook taskHook = new LVRAuctionTaskHook(
            context.lvrAuctionHook,
            context.serviceManager
        );
        console.log("LVRAuctionTaskHook deployed to:", address(taskHook));
        console.log("Connected to main LVR Hook at:", context.lvrAuctionHook);
        ...
        // - Connect with main LVR Auction Hook

        vm.stopBroadcast();

        // Output the deployed contracts
        Output[] memory outputs = new Output[](1);
        outputs[0] = Output("LVRAuctionTaskHook", address(taskHook));
    ...
    function _readContext(string memory environment, string memory _context) internal view returns (Context memory) {
        ...
        context.lvrAuctionHook = contextJson.readAddress(".l2.lvrAuctionHook");  // Main hook address
